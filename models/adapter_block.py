...
# adapterBlock的实现，和单纯的vit相比，增加了adapter
class AdapterBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.norm1(x)
        x = self.attn(x)
        # 属于在论文里做完multi-head attention之后的操作
        x = self.Space_Adapter(x)

        # Reverse window partition
        if self.window_size > 0:
            x = window_unpartition(x, self.window_size, pad_hw, (H, W))
        # shortcut为初始的输入，x为经过layer norm, multi-head attention,adapter的输出
        x = shortcut + x
        # layer normalization
        xn = self.norm2(x)
        # 对apapter进行scaling，这个adapter和mlp的输出结果
